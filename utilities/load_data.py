...
def get_dataset_func(samplingfunction, validityfunction, rangearr):
    def sample(samplingfunction = samplingfunction, validityfunction=validityfunction, rangearr=rangearr):
        ...
        return distribution, positive, negative
    return sample

def gen_dataset(datasetfunction, holdout_frac, scaling):
    distribution, negative, positive = datasetfunction()
    # Scale if desired
    if scaling:
        scaler = StandardScaler()
    ...
    positive = distribution  # Temporarily setting this
    return distribution, positive, negative, holdout, scaler
