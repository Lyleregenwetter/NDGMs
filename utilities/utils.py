...
def fit_and_generate(functions, methods, numinst, numanim, numgen, scaling, obj_status, conditional_status, holdout = 0, resample_dataset=False):
    ...
    for func in range(len(functions)):
        ...
        #Generate the data
        distribution_scaled, positive_scaled, negative_scaled, holdout_scaled, scaler = load_data.gen_dataset(datasetfunction, holdout, scaling)
        ...
        for inst in range(numinst):
            ...
            if obj_status:
                y_distribution = eval_obj(distribution, objectives)
            else:
                y_distribution = None
            #Evaluate condition value for all datapoints
            if conditional_status:
                c_distribution = eval_obj(distribution, [cond_func])
            else:
                c_distribution = None
            ...
            for i in range(len(methods)): 
                ...
                # x_fake_scaled, cps = method(distribution_scaled, negative_scaled, positive_scaled, y_distribution, c_distribution, numgen, numanim, np.full(numgen, np.full(numgen, cond)), savedir = modeldir)
            ...
            ps_s[func][inst] = positive_scaled
...
def save_generated(generated, checkpoint_steps, xs_s, ps_s, ns_s, hs_s, scaler_s, timestr):
    ...
    os.mkdir(f"Results/{timestr}/Positive_samples")
    ...
    for i in range(len(xs_s)):
        ...
        for j in range(len(xs_s[i])):
            ...
            np.save(f"Results/{timestr}/Positive_samples/Problem_{i}_Instance_{j}.npy", ps_s[i][j])
...
def load_generated(timestr, numinst, numfunc):
    ...
    for i in range(numfunc):
        ...
        for j in range(numinst):
            ...
            ps_s[i][j] = np.load(f"Results/{timestr}/Positive_samples/Problem_{i}_Instance_{j}.npy")
    ...
    return generated, checkpoint_steps, xs_s, ps_s, ns_s, hs_s, scaler_s

def gen_background_plot(validityfunction, rangearr):
    xinc = (rangearr[0,1]-rangearr[0,0])/200
    yinc = (rangearr[1,1]-rangearr[1,0])/200
    xx, yy = np.mgrid[rangearr[0,0]:rangearr[0,1]+xinc:xinc, rangearr[1,0]:rangearr[1,1]+yinc:yinc]
...
def plot(ax, rangearr, xx, yy, Z, x, y, x2,y2, boundary=0.0, plottype = "generated", validity_status=0, color = "red", target=None):
...
def plot_all(timestr, functions, methods, numinst, scaling, validity_status, obj_status, conditional_status, cond_dist, color="red", plotobjs=None, plot_steps=True):
    plt.ioff()
    generated, checkpoint_steps, xs_s, ps_s, ns_s, hs_s, scaler_s= load_generated(timestr, numinst, len(functions))
    ...
    for i in range(len(functions)):
        if generated[i].shape[4]==2:
            functions_to_visualize.append(i)
    if functions_to_visualize==[]:
        return 
    ...
    for inst in range(numinst):
        for step in range(steps):
            ...
            #Initialize subplots
            fig, ax = plt.subplots(len(functions_to_visualize), plots_in_row, figsize=(10*plots_in_row+0.2, 10*len(functions_to_visualize)+0.2), facecolor='white')
            fig.subplots_adjust(hspace=0.03, wspace=0.03)
            #Loop over problems to test
            for func_idx, func in enumerate(functions_to_visualize):
                #Unpack problem info
                datasetfunction, validityfunction, objectives, rangearr, cond_func, cond = functions[func]

                distribution_scaled = xs_s[func][inst]
                positive_scaled = ps_s[func][inst]
                ...
                xx, yy, Z = gen_background_plot(validityfunction, rangearr)
                plot(fig.axes[plots_in_row*func_idx], rangearr, xx, yy, Z, distribution[:,0], distribution[:,1], None, None, validity_status=validity_status, color=color, plottype = "dataset")
                if func_idx==0:
                    fig.axes[plots_in_row*func_idx].set_title("Original Distribution Data")
                fig.axes[plots_in_row*func_idx].set_ylabel(f"Problem {func+1}", fontsize=36)

                #---------------------------Negative Data Plots---------------------------------

                if validity_status==1:
                    plot(fig.axes[plots_in_row*func_idx+1], rangearr, xx, yy, Z, negative[:,0], negative[:,1], None, None, validity_status=validity_status, color=color, plottype = "invalid")
                if func_idx==0:
                    fig.axes[plots_in_row*func_idx+1].set_title("Original Negative Data")
                ...
                if obj_status:
                    ...
                    for i in range(num_objectives):
                        xx_o, yy_o, Z_o = gen_background_plot(objectives[i], rangearr)
                        obj_idx = plots_in_row*func_idx+1+validity_status+i
                        plot(fig.axes[obj_idx], rangearr, xx_o, yy_o, Z_o, distribution[:,0], distribution[:,1], None, None, 
                             plottype = "objective", validity_status=validity_status, color=color, target=plotobjs[i])
                        if func_idx==0:
                            fig.axes[obj_idx].set_title(f"Objective {i+1}")
                else:
                    num_objectives = 0
                ...
                if conditional_status:
                    ...
                    xx_o, yy_o, Z_o = gen_background_plot(cond_func, rangearr)
                    ...
                    if func_idx==0:
                        fig.axes[cond_idx].set_title("Condition Function")
                ...
                if cond_dist:
                    ...
                    if func_idx==0:
                        fig.axes[cond_idx+1].set_title(f"Condition-Satisfying Subset")

                #Loop over methods to test
                for i in range(len(methods)): 
                    if plot_steps:
                        epoch = f": Epoch {checkpoint_steps[i][step]+1}"
                    else:
                        epoch = ""
                    ...
                    if objectives:
                        y_fake = eval_obj(x_fake, objectives)
                    else:
                        y_fake = None

                    #---------------------------Generated over Full Dataset with or without validity------------------

                    res_idx=plots_in_row*func_idx+1+validity_status+max_obj*obj_status+i*(1+cond_dist)+conditional_status+cond_dist
                    if validity_status:
                        labels, _ = evaluation.evaluate_validity(x_fake, validityfunction)
                        labels = labels.astype(bool)
                        plot(fig.axes[res_idx], rangearr, xx, yy, Z, x_fake[:,0][labels], x_fake[:,1][labels], x_fake[:,0][~labels], x_fake[:,1][~labels], plottype="generated", validity_status = validity_status, color=color)                
                    else:
                        plot(fig.axes[res_idx], rangearr, xx, yy, Z, x_fake[:,0], x_fake[:,1], distribution[:,0], distribution[:,1], 
                             plottype="generated", validity_status = validity_status, color=color)
                    if func_idx==0:
                        fig.axes[res_idx].set_title(f"{methods.index[i]}{epoch}")
                        
                    #---------------------------Generated over Conditional Subset------------------------------------

                    if cond_dist:
                        plot(fig.axes[res_idx+1], rangearr, xx, yy, Z, x_fake[:,0], x_fake[:,1], distribution_mask[:,0], distribution_mask[:,1],
                             validity_status=validity_status, color=color, plottype = "conditional")
                        if func_idx==0:
                            fig.axes[res_idx+1].set_title(f"{methods.index[i]}{epoch}")
...
def score(timestr, functions, methods, metrics, numinst, scaling, cond_dist, scorebars, plotobjs=None, style=True, plotscores=False, score_instances=True):
            ...
            for func in range(len(functions)):
                ...
                if objectives:
                    ...
                    if "Rediscovery" in metrics:
                        holdout_y = eval_obj(distribution, objectives)
                else:
                    y_distribution=None
                    ...
                    if np.isnan(x_fake_scaled).any():
                        ...
                        for j in range(len(metrics)):
                            if metrics.values[j][1]=="Validity":
                                ...
                                c_gen = eval_obj(x_fake, [cond_func])
                                allscores=None
    ...
    #[Functon, Methods, Metrics, Instances, Timesteps]

    reset_folder(f"Results/{timestr}/Scores")
    os.mkdir(f"Results/{timestr}/Scores/By_problem")
    os.mkdir(f"Results/{timestr}/Scores/By_metric")
    if plotscores:
    ...
    for i in range(np.shape(scores)[0]):
        ...
        if score_instances:
            for j in range(np.shape(scores)[3]):
                ...
                write_excel_or_csv(instance_df, style, f"Results/{timestr}/Scores/By_problem/problem_{i+1}_instance{j+1}_scores", f"Problem {i+1} Inst {j+1} Scores:", [v[0] for v in metrics.values], True)
        if numinst>1:
            scoredf = append_error(scoredf_raw, stds)
        else:
            scoredf = scoredf_raw.copy()
        write_excel_or_csv(scoredf, style, f"Results/{timestr}/Scores/By_problem/problem_{i+1}_scores", f"Problem {i+1} Scores:", [v[0] for v in metrics.values], True)

        medianscores = np.median(scores[i], axis=(2))
        scoredf_raw = pd.DataFrame(medianscores, index=methods.index, columns = metrics.index).transpose()
        write_excel_or_csv(scoredf_raw, style, f"Results/{timestr}/Scores/By_problem/problem_{i+1}_median_scores", f"Problem {i+1} Median Scores:", [v[0] for v in metrics.values], True)
    
    for i in range(np.shape(scores)[2]):
        meanscores = np.mean(scores[:,:,i,:], axis=(2))
        stds = np.std(scores[:,:,i,:], axis=(2)).transpose()
        problemnames = [f"Problem {i+1}" for i in range(len(functions))]
        scoredf_raw = pd.DataFrame(meanscores, index=problemnames, columns = methods.index)
        if score_instances:
            for j in range(np.shape(scores)[3]):
                instance_df = pd.DataFrame(scores[:, :,i,j], index=problemnames, columns = methods.index)
                write_excel_or_csv(instance_df, style, f"Results/{timestr}/Scores/By_metric/{metrics.index[i]}_instance{j+1}_scores", f"{metrics.index[i]} Inst {j+1} Scores:", [metrics.values[i][0]]*len(functions), True)
        if numinst>1:
            print(stds)
            print(scoredf_raw)
            scoredf = append_error(scoredf_raw, stds)
            ...
            scoredf = scoredf_raw.copy()
        write_excel_or_csv(scoredf, style, f"Results/{timestr}/Scores/By_metric/{metrics.index[i]}_scores", f"{metrics.index[i]} Scores:", [metrics.values[i][0]]*len(functions), True)

        medianscores = np.median(scores[:,:,i,:], axis=(2))
        scoredf_raw = pd.DataFrame(medianscores, index=problemnames, columns = methods.index)
        write_excel_or_csv(scoredf_raw, style, f"Results/{timestr}/Scores/By_metric/{metrics.index[i]}_median_scores", f"{metrics.index[i]} Median Scores:", [metrics.values[i][0]]*len(functions), True)
    
    if len(functions)>1:
        #Score Rankings
        rankarr = np.argsort(np.argsort(np.mean(scores, axis=(3)), axis=1), axis=1)
        direction_mask = np.array([direction=="minimize" for direction in [v[0] for v in metrics.values]])
        rankarr = rankarr*direction_mask + (len(methods)-1-rankarr)*(1-direction_mask)
        average_ranks = np.mean(rankarr, axis=0)
        rankdf = pd.DataFrame(average_ranks, index=methods.index, columns = metrics.index).transpose()
        write_excel_or_csv(rankdf, style, f"Results/{timestr}/Scores/average_ranks", "Average Ranks:", ["minimize" for v in metrics.values], True)
        ...
        scoredf_raw = pd.DataFrame(meanscores, index=methods.index, columns = metrics.index).transpose()
        scoredf = append_error(scoredf_raw, stds)
        write_excel_or_csv(scoredf, style, f"Results/{timestr}/Scores/mean_scores", "Mean scores:", [v[0] for v in metrics.values], True)

        medianscores = np.median(scores, axis=(0,3))
        scoredf_raw = pd.DataFrame(medianscores, index=methods.index, columns = metrics.index).transpose()
        write_excel_or_csv(scoredf_raw, style, f"Results/{timestr}/Scores/median_scores", "Median scores:", [v[0] for v in metrics.values], True)

def write_excel_or_csv(df, style, filename, index_label, highlight_rule, print_df=True):
    if style:
        df = highlight_best(df, df, highlight_rule)
        df.to_excel(filename + ".xlsx", index_label=index_label)
    if not style:
        df.to_csv(filename + ".csv", index_label=index_label)
    if print_df:
        display(df)
...
def append_error(df, stds):
    for i in range(len(df.index)):
        for j in range(len(df.columns)):
            df.iloc[i,j] = f"{df.iloc[i,j]:.5f}\u00B1{stds[j,i]:.5f}"
    return df
